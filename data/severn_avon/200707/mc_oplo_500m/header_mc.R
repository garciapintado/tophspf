...
# numeric: Monte Carlo generation | character: system filename of a map
GP <- list()
GP$lzsn   <- c(0.25,40.0)        # [mm]    [.25,2500.] lower zone nominal storage                                                           PWAT-PARM2
GP$infilt <- c(0.34,3.4)         # [mm/hr] [.0025,2500.] index to the infiltration capacity of the soil                                           "
GP$lsur   <- c(100000,120000)    # [m]     [.3,none] foo values: length of assumed overland flow plane " (specified below for every subwatershed)
GP$slsur <- "slpfilled.tif"      # [0/1] [.000001,10.] distributed slope of the overland flow plane (if path is included, it is relative to dsnsim)
...
GP$uzsn   <- c(20.0,50.0)        # [mm]    none is default [.25,250.] upper soil zone nominal storage (hrus's distributed)             "
GP$nsur   <- c(0.06,0.1)         # [cplx]  0.1 is default [0.001,1.0] manning coefficient for overland flow                            "
GP$intfw  <- c(0.5,3.0)          # [-]     none is default [0.0,none] interflow inflow parameter                                       "
GP$irc    <- c(0.1,0.9)          # [1/day] none is default [1.0E-30,0.999] interflow recession parameter. Under zero inflow,
...
GP$surs <- c(0.0,3.0)    # [mm] surface detention storage                    PWAT-STATE1
GP$ifws <- c(0.0,0.5)    # [mm] interflow storage                            PWAT-STATE1
GP$uzs  <- c(1.0,5.0)    # [mm] upper zone storage                           PWAT-STATE1
GP$lzs  <- c(0.0,40.0)   # [mm] lower zone storage                           PWAT-STATE1
GP$agws <- c(1.5,3.5)    # [mm] active groundwater storage                   PWAT-STATE1
GP$gwvs <- c(0.0,5.0)    # [mm] groundwater variable storage index           PWAT-STATE1

MCflags <- rep(FALSE,length(GP))
names(MCflags) <- names(GP)
MCflags[c('lzsn','infilt','lsur','agwr','infexp','cepsc','uzsn','nsur','intfw','irc','lzetp',
          'ceps','surs','ifws','uzs','lzs','agws','gwvs')] <- TRUE   # parameters & initial conditions to randomize for MC

# subwatershed specific parameters and inheritance rules
npar <- length(GP)
hrusSGDF <- readGDAL(file.path(dsnsim,RF$hrusf))
nhrus <- max(hrusSGDF@data[,1], na.rm=TRUE)
dx <- hrusSGDF@grid@cellsize[1]

WP <- list()                                        # specific subwatershed data
swareas <- rep(0,nhrus)
for (iw in 1:nhrus) {
  swareas[iw] <- sum(hrusSGDF@data[,1]==iw)*dx^2    # [m2] subwatershed areas  
  WP[[iw]] <- list()
  for (ipar in 1:npar) {
    WP[[iw]] <- GP                                  # inherit from global parameters
  }
  WP[[iw]]$lsur <- sqrt(swareas[iw]) / 2            # [m] rough 'half-book' estimate for 'smdis'
  WP[[iw]]$lsur <- WP[[iw]]$lsur*c(1.0,1.2) 
}
