...
GP <- list()
GP$lzsn   <- 18.5                # [mm]    [.25,2500.] lower zone nominal storage                                                           PWAT-PARM2
...
GP$uzsn   <-    6.54             # [mm]    none is default [.25,250.] upper soil zone nominal storage (hrus's distributed)             "
GP$nsur   <- c(0.035,0.04)       # [cplx]  0.1 is default [0.001,1.0] manning coefficient for overland flow                            "
GP$intfw  <-    2.66             # [-]     none is default [0.0,none] interflow inflow parameter                                       "
GP$irc    <- c(0.70,0.80)        # [1/day] none is default [1.0E-30,0.999] interflow recession parameter. Under zero inflow,
...
MC <- data.frame(flag=rep(FALSE,length(p)), dis='runif', min=NA, max=NA, # range [min,max] just for initial generation
                   row.names=names(p), stringsAsFactors=FALSE)

# what is uncertain:
MC[c('infilt','agwr','infexp','cepsc','nsur','irc','lzetp',     # parameters
     'ceps','surs','ifws','uzs','lzs','agws','gwvs'),'flag'] <- TRUE   # initial conditions
