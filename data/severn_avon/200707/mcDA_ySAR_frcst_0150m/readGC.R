...
GC$simtime      <- as.numeric(difftime(RF$endT,RF$staT,units='secs'))                                                         # integration time (time until next assimilation time)
GC$output_times <- seq(0.0, GC$simtime, by=900)                                        # e.g. including overpass times
GC$gauges   <- RF$gauges                                                               # instance of SpatialPointsDataFrame
