...
GP$uzsn   <- c(0.25,50.0)        # [mm]    none is default [.25,250.] upper soil zone nominal storage (hrus's distributed)             "
...
SW <- list()                     # specific subwatershed data
for (iw in 1:nhrus){
 SW[[iw]] <- list()
 SW[[iw]]$area <-      sum(hrusSGDF@data[,1]==iw)*dx^2    # [m2] subwatershed areas
 SW[[iw]]$lsur <- sqrt(sum(hrusSGDF@data[,1]==iw)*dx^2)/2 # [m] rough estimate for 'smdis'
}
