...
Module ModuleEnterData
    ...
    interface  GetData                              ! user-defined generic procedure
    ...
    interface WriteDataLine                         ! user-defined generic procedure
   ...
   !Parameter-----------------------------------------------------------------
    ...
    ! and stores them in string "ObjEnterData" !
    ...
    SUBROUTINE ConstructEnterData(EnterDataID, FileName, ErrorMessage, FORM, STAT)
        ...
        IF (.NOT. ModuleIsRegistered(mENTERDATA_)) THEN           ! both, function and global parameter mENTERDATA_ = 3 defined in ModuleGlobalData
            NULLIFY (FirstEnterData)
            CALL RegisterModule (mENTERDATA_)                     ! in ModuleGlobalData simply sets RegisteredModules(iModule) to .TRUE.
        END IF

        STAT_ = UNKNOWN_

        CALL Ready(EnterDataID, ready_)                           ! in this module. nullifies and then sets Me pointer so that Me%InstanceID == EnterDataID
...
cd0 :   IF (ready_ .EQ. OFF_ERR_) THEN
            ...
            call AllocateInstance                                                              ! in this module allocate the unnamed target of Me
...
if8 :       IF      (FORM_ .EQ. FORMATTED_  ) THEN                                                 ! default
...
do3 :       DO i = 1, Me%BufferSize
    ...
    SUBROUTINE AllocateInstance
        ...
        Me%InstanceID = RegisterNewInstance (mENTERDATA_)                         ! RegisterNewInstance integer function in ModuleGlobalData
...
cd1 :   if (ready_ .EQ. IDLE_ERR_) then
    ...
    !
    !     This SUBROUTINE reads the name of the file FILE_NAME,
    !     defined by KEYWORD in input file FilesName.
    !
    !     If the the input FILE_NAME is not found in FilesName file, the
    !     corresponding outfile is automatically named and created.
    !
    !     In the case a file_name is automatically created, the extension is a length-3 string
    !
    ...
    SUBROUTINE ReadFileName(KEYWORD, FILE_NAME, Message, TIME_END, Extension, FilesInput, MPI_ID, DD_ON, STAT)
        ...
        type(T_Time),       optional, intent(IN ) :: TIME_END                                    !if available, simulation instant time used to generate file_name
        ...
        FileNomfich = TRIM(FilesName)                                           ! FilesName global variable defined in ModuleGlobalData & set in calling main program units
...
cd1 :       IF (present(Message)) THEN
                ...
                CALL GetData(FILE_NAME, ObjEnterData, flag, SearchType = FromFile_, text = Message, keyword = KEYWORD)  ! INTERFACE to SUBROUTINE ReadString_New
...
do1 :   DO i = StartSearch, EndSearch
            ...
            IF (KeywordsEqual(trim(ReadKeyWord), trim(keyword), CaseSensitiveAux)) THEN
                IF (ValidDelimiter) THEN
                    ...
                    write(*,*)'ValidDelimiter = ', ValidDelimiter
                    write(*,*)'Keyword        = ', keyword
                    write(*,*)'delimiter_pos  = ', string%delimiter_pos
...
if9 :       IF (present(SearchType)) THEN
            ...
            ELSE IF (present(Buffer_Line)) THEN
...
cd1 :       IF (flag == 1) THEN                                        ! i.e. if keyword found by LookForLineInBlock, so string is available
...
100     continue
    ...
    subroutine ReadTime(value,                                                &
        ...
        integer,            intent(IN ), optional   :: Buffer_Line
        character(LEN = *), intent(IN ), optional   :: keyword
        type(T_Time),       intent(IN ), optional   :: Default
        character(LEN = *), intent(IN ), optional   :: ClientModule
        logical           , intent(IN ), optional   :: CaseSensitive
        integer,            intent(OUT), optional   :: STAT
        ...
        call Ready(EnterDataID, ready_)
        write(*,*)'ready to read keyword: ', keyword
...
cd1:    IF (EnterDataID > 0) THEN
            CALL LocateObjEnterData (EnterDataID)                              ! in this module. set the point Me to Me%InstanceID == EnterDataID
            ready_ = VerifyReadLock (mENTERDATA_, Me%InstanceID)               ! function in ModuleGlobalData
        ELSE
            ready_ = OFF_ERR_
        END IF cd1
