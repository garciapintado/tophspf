...
! DATE          : 2014
! REVISION      : Javier Garcia-Pintado - v1.0
...
MODULE ModuleGlobalData
  USE, INTRINSIC      :: Iso_C_Binding ! Fortran 2003 intrinsic module (see gcc.gnu.org 10.2)
  USE nrtype
...
!________________________________________________________________________________________________________________________
    ...
    integer, parameter ::  mSEAGRASSWATERINTERAC_   = 81     ! Isabella
    integer, parameter ::  mSEAGRASSSEDIMINTERAC_   = 82     ! Isabella
    ...
    integer, parameter ::  mAnalytical_LDS_         = 87
    integer, parameter ::  mPressureDifferences_    = 88
    integer, parameter ::  mHNS_                    = 89
    integer, parameter ::  mGlueWW3_OBC_            = 90
    integer, parameter ::  mSnow_                   = 91
    INTEGER, PARAMETER ::  mHSPFPWATER_             = 92     ! Javier
    ...
    type (T_Module), dimension(MaxModules), parameter  :: MohidModules =  (/             &
        T_Module(mGLOBALDATA_            , "GlobalData"),            T_Module(mTIME_                   , "Time"),                  &        !  1, 2
        T_Module(mENTERDATA_             , "EnterData"),             T_Module(mFUNCTIONS_              , "Functions"),             &        !  3, 4
        T_Module(mLUD_                   , "LUD"),                   T_Module(mWATERQUALITY_           , "WaterQuality"),          &        !  5, 6
        T_Module(mGRIDDATA_              , "GridData"),              T_Module(mHDF5_                   , "HDF5"),                  &        !  7, 8
        T_Module(mHORIZONTALGRID_        , "HorizontalGrid"),        T_Module(mHORIZONTALMAP_          , "HorizontalMap"),         &        !  9,10
        T_Module(mGEOMETRY_              , "Geometry"),              T_Module(mSTOPWATCH_              , "StopWatch"),             &        ! 11,12
        T_Module(mADVECTIONDIFFUSION_    , "AdvectionDiffusion"),    T_Module(mMAP_                    , "Map"),                   &        ! 13,14
        T_Module(mTIMESERIE_             , "TimeSerie"),             T_Module(mSOIL_                   , "Soil"),                  &        ! 15,16
        T_Module(mBOXDIF_                , "BoxDif"),                T_Module(mASSIMILATION_           , "Assimilation"),          &        ! 17,18
        T_Module(mSTATISTIC_             , "Statistic"),             T_Module(mDISCHARGES_             , "Discharges"),            &        ! 19,20
        T_Module(mATMOSPHERE_            , "Atmosphere"),            T_Module(mHYDROINTEGRATION_       , "HydroIntegration"),      &        ! 21,22
        T_Module(mHYDRODYNAMICFILE_      , "HydrodynamicFile"),      T_Module(mTOGA_                   , "Toga"),                  &        ! 23,24
        T_Module(mGAUGE_                 , "Gauge"),                 T_Module(mTRIANGULATION_          , "Triangulation"),         &        ! 25,26
        T_Module(mGOTM_                  , "GOTM"),                  T_Module(mTURBGOTM_               , "TurbGOTM"),              &        ! 27,28
        T_Module(mTURBULENCE_            , "Turbulence"),            T_Module(mOPENBOUNDARY_           , "OpenBoundary"),          &        ! 29,30
        T_Module(mHYDRODYNAMIC_          , "Hydrodynamic"),          T_Module(mFREEVERTICALMOVEMENT_   , "FreeVerticalMovement"),  &        ! 31,32
        T_Module(mSEDIMENTQUALITY_       , "SedimentQuality"),       T_Module(mINTERFACE_              , "Interface"),             &        ! 33,34
        T_Module(mWATERPROPERTIES_       , "WaterProperties"),       T_Module(mOIL_                    , "Oil"),                   &        ! 35,36
        T_Module(mJET_                   , "Jet"),                   T_Module(mLAGRANGIAN_             , "Lagrangian"),            &        ! 37,38
        T_Module(mMODEL_                 , "Model"),                 T_Module(mCONSOLIDATION_          , "Consolidation"),         &        ! 39,40
        T_Module(mSEDIMENTPROPERTIES_    , "SedimentProperties"),    T_Module(mLIFE_                   , "Life"),                  &        ! 41,42
        T_Module(mCEQUALW2_              , "CEQUALW2"),              T_Module(mBASINGEOMETRY_          , "BasinGeometry"),         &        ! 43,44
        T_Module(mRUNOFF_                , "RunOff"),                T_Module(mDRAINAGENETWORK_        , "DrainageNetwork"),       &        ! 45,46
        T_Module(mINTERFACEWATERAIR_     , "InterfaceWaterAir"),     T_Module(mINTERFACESEDIMENTWATER_ , "InterfaceWaterSediment"),&        ! 47,48
        T_Module(mLIGHTEXTINCTION_       , "LightExtinction"),       T_Module(mRIVERHYDRODYNAMIC_      , "RiverHydrodynamic"),     &        ! 49,50
        T_Module(mFILLMATRIX_            , "FillMatrix"),            T_Module(mWAVES_                  , "Waves"),                 &        ! 51,52
        T_Module(mBASIN_                 , "Basin"),                 T_Module(mSOILPROPERTIES_         , "SoilProperties"),        &        ! 53,54
        T_Module(mINFILTRATION_          , "Infiltration"),          T_Module(mSOILPLANTAIR_           , "SoilPlantAir"),          &        ! 55,56
        T_Module(mSOILMACROPORES_        , "SoilMacropores"),        T_Module(mSAND_                   , "Sand"),                  &        ! 57,58
        T_Module(mMACROPOREPROPERTIES_   , "MacroporeProperties"),   T_Module(mPOROUSMEDIA_            , "PorousMedia"),           &        ! 59,60
        T_Module(mSWAT_                  , "ModuleSwat"),            T_Module(mPROFILE_                , "Profile"),               &        ! 61,62
        T_Module(mBENTHOS_               , "Benthos"),               T_Module(mCLIMATOLOGY_            , "Climatology"),           &        ! 63,64
        T_Module(mFIREINDEX_             , "FireIndex"),             T_Module(mINTERPOLATION_          , "Interpolation"),         &        ! 65,66
        T_Module(mVEGETATION_            , "Vegetation"),            T_Module(mRESERVOIROPTIMIZATION_  , "ReservoirOptimization"), &        ! 67,68
        T_Module(mMACROALGAE_            , "MacroAlgae"),            T_Module(mBFM_                    , "BFM"),                   &        ! 69,70
        T_Module(mNETCDF_                , "NETCDF"),                T_Module(mSEQUENTIALASSIMILATION_ , "SequentialAssimilation"),&        ! 71,72
        T_Module(mPOROUSMEDIAPROPERTIES_ , "PorousMediaProperties"), T_Module(mPHREEQC_                , "PhreeqC"),               &        ! 73,74
        T_Module(mRUNOFFPROPERTIES_      , "RunoffProperties"),      T_Module(mCHAINREACTIONS_         , "ChainReactions"),        &        ! 75,76
        T_Module(mCUDA_                  , "Cuda"),                                                                                &        ! 77
        T_Module(mField4D_               , "Field4D"),               T_Module(mBENTHICECOLOGY_         , "BenthicEcology"),        &        ! 78,79
        T_Module(mWWTPQ_                 , "WWTPQ") ,                                                                              &        ! 80
        T_Module(mSEAGRASSWATERINTERAC_  , "SeagrassWaterInteraction"),                                                            &        ! 81
        T_Module(mSEAGRASSSEDIMINTERAC_  , "SeagrassSedimInteraction"), T_Module(mBivalve_             , "BivalveModel"),          &        ! 82,83
        T_Module(mTimeSeriesAnalyser_    , "TimeSeriesAnalyser"),       T_Module(mNetworkStatistics_   , "NetworkStatistics"),     &        ! 84,85
        T_Module(mTimeSeriesOperator_    , "TimeSeriesOperator"),       T_Module(mAnalytical_LDS_      , "Analytical_LDS"),        &        ! 86,87
        T_Module(mPressureDifferences_   , "PressureDifferences"),      T_Module(mHNS_                 , "HNS"           ),        &        ! 88,89
        T_Module(mGlueWW3_OBC_           , "GlueWW3_OBC"),           T_Module(mSnow_                   , "Snow"          ),        &        ! 90,91
        T_Module(mHSPFPWATER_,             "HSPFPWATER") /)                                                                                 ! 92
    ...
    character(LEN=PathLength)                                       :: dsn                = '.'                ! main input folder where FilesName resides
...
214 format(i8, ', DT: ', f9.3, a)
    ...
    SUBROUTINE StartupHydro(modelname)
        ...
        !numbers is generated. This is important for debug purposes
        ...
        write(*, *)"-------------------------- HYDROLAND -------------------------"
        write(*, *)
        write(*, *)"      AUTHOR   : Javier Garcia-Pintado      "
        write(*, *)"      WWW      : http://www.nceo.org"
        write(*, *)
        write(*, *)
        ...
        CALL UnitsManager(ErrorFileID, OPEN_FILE, STAT = STAT_CALL)                               ! in ModuleGlobalData. PRIV mod variables: ErrorFileID, OPEN_FILE=1 (par)
        IF (STAT_CALL .NE. SUCCESS_) STOP 'StartupHydro - GlobalData - ERR01'
...
do1:    DO
        ...
        !Gets a unit for the (output) UsedKeyWordFile
        CALL UnitsManager(UsedKeyFileID, OPEN_FILE, STAT = STAT_CALL)
        IF (STAT_CALL .NE. SUCCESS_) STOP 'StartupHydro - GlobalData - ERR02'
...
do2:    DO
    ...
    END SUBROUTINE StartupHydro

    !--------------------------------------------------------------------------

    SUBROUTINE ShutdownHydro (ModelName, ElapsedSeconds, TotalCPUTime,            &
                              WorkCycleElapsed, WorkCycleCPUTime)

        !Arguments-------------------------------------------------------------
        CHARACTER(len=*),    INTENT(IN)                 :: ModelName
        REAL, INTENT(IN)                                :: ElapsedSeconds, TotalCPUTime
        REAL, optional, INTENT(IN)                      :: WorkCycleElapsed, WorkCycleCPUTime
        ...
        !Write a comment in the error and messages to indicate that model run went ok
        WRITE(unit=ErrorFileID, fmt=*)"SUCCESSFUL  : 1"

        !Close Errors and messages
        CALL UnitsManager(ErrorFileID, CLOSE_FILE, STAT = STAT_CALL)
        if (STAT_CALL .NE. SUCCESS_) stop 'ShutdownHydro - GlobalData - ERR01'

        CALL UnitsManager(UsedKeyFileID, CLOSE_FILE, STAT = STAT_CALL)
        if (STAT_CALL .NE. SUCCESS_) stop 'ShutdownHydro - GlobalData - ERR02'
