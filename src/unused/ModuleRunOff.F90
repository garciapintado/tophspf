!------------------------------------------------------------------------------
!        UoR, NCEO, Hydroland modelling system
!------------------------------------------------------------------------------
!
! TITLE         : tophspf model
! PROJECT       : SINATRA
...
! AFFILIATION   : UoR
! DATE          : Jan 2015
! REVISION      : Javier Garcia-Pintado
...
!        type(T_PointF)                             :: Location
    ...
    type  T_RunOff
        ...
        type (T_Time)                               :: EndTime
        real(DP), dimension(:,:), pointer           :: myWaterLevel             => null()
        real(DP), dimension(:,:), pointer           :: myWaterColumn            => null()
        ...
        real(DP), dimension(:,:), pointer            :: myWaterVolume            => null()
        real(DP), dimension(:,:), pointer            :: myWaterColumnOld         => null() !OldColumn from Basin
        real(DP), dimension(:,:), pointer            :: myWaterColumnAfterTransport => null() !for property transport
        real(DP), dimension(:,:), pointer            :: myWaterVolumePred        => null() !to avoid negative collumns
        real(DP), dimension(:,:), pointer            :: myWaterVolumeOld         => null()
        ...
        real(DP), dimension(:,:), pointer            :: lFlowX, lFlowY           => null() !Instantaneous OverLandFlow (LocalDT   )
        real(DP), dimension(:,:), pointer            :: iFlowX, iFlowY           => null() !Integrated    OverLandFlow (AfterSumDT)
        real(DP), dimension(:,:), pointer            :: FlowXOld, FlowYOld       => null() !Flow From previous iteration
        real(DP), dimension(:,:), pointer            :: InitialFlowX, InitialFlowY => null() !Initial Flow of convergence
...
!        logical                                     :: CheckDecreaseOnly    = .false.
        ...
        real(DP)                                     :: BoundaryFlowVolume        = 0.0 !m3 => positive if flow is towards boundary.
        real(DP)                                     :: VolumeStoredInSurface     = 0.0
        real(DP)                                     :: VolumeStoredInStormSystem = 0.0
        real(DP)                                     :: TotalDischargeFlowVolume  = 0.0

        logical                                     :: Continuous          = .false.
        logical                                     :: StopOnWrongDate     = .true.

        real(DP)                                     :: TotalStoredVolume  = 0.
...
!                        enddo
   ...
   10   continue
    ...
    subroutine GetOverLandFlow (ObjRunOffID, FlowX, FlowY, STAT)
        ...
        !Arguments-------------------------------------------------------------
        integer                                         :: ObjRunOffID
        real(DP), dimension(:, :), pointer               :: FlowX, FlowY
...
cd1 :   if ((ready_ .EQ. IDLE_ERR_     ) .OR. &
    ...
    !--------------------------------------------------------------------------
        ...
        !Arguments-------------------------------------------------------------
        integer                                         :: ObjRunOffID
        real(DP), dimension(:, :), pointer               :: WaterLevel
        integer, intent(OUT), optional                  :: STAT
...
cd1 :   if ((ready_ .EQ. IDLE_ERR_     ) .OR. &
    ...
    subroutine GetRunoffWaterColumn (ObjRunOffID, WaterColumn, STAT)
        ...
        !Arguments-------------------------------------------------------------
        integer                                         :: ObjRunOffID
        real(DP), dimension(:, :), pointer               :: WaterColumn
        integer, intent(OUT), optional                  :: STAT
...
cd1 :   if ((ready_ .EQ. IDLE_ERR_     ) .OR. &
    ...
    subroutine GetRunoffWaterColumnOld (ObjRunOffID, WaterColumnOld, STAT)
        ...
        !Arguments-------------------------------------------------------------
        integer                                         :: ObjRunOffID
        real(DP), dimension(:, :), pointer               :: WaterColumnOld
        integer, intent(OUT), optional                  :: STAT
...
cd1 :   if ((ready_ .EQ. IDLE_ERR_     ) .OR. &
    ...
    subroutine GetRunOffStoredVolumes (ID, Surface, StormSystem, STAT)
        ...
        !Arguments-------------------------------------------------------------
        integer                                         :: ID
        real(DP), intent(OUT), optional                  :: Surface
        real(DP), intent(OUT), optional                  :: StormSystem
        integer, intent(OUT), optional                  :: STAT
    ...
    subroutine GetRunOffBoundaryFlowVolume (ID, BoundaryFlowVolume, STAT)
        ...
        !Arguments-------------------------------------------------------------
        integer                                         :: ID
        real(DP), intent(OUT)                            :: BoundaryFlowVolume
        integer, intent(OUT), optional                  :: STAT
    ...
    subroutine ModifyWaterDischarges (LocalDT)
        ...
        do iDis = 1, nDischarges
            ...
            if (k == 0) then
                ...
                !real(DP) to real as expected in GetDischargeWaterFlow
...
cd1:    if (RunOffID > 0) then
    ...
    logical function GetPondedWaterColumn(RunOffID, nComputePoints, waterColumn)
        ...
        real(DP), dimension(nComputePoints)          :: waterColumn
    ...
    logical function GetInletInFlow(RunOffID, nComputePoints, inletInflow)
        ...
        integer                                     :: RunOffID
        integer                                     :: nComputePoints
        real(DP), dimension(nComputePoints)          :: inletInflow
    ...
    logical function SetStormWaterModelFlow(RunOffID, nComputePoints, overlandToSewerFlow)
        ...
        integer                                     :: RunOffID
        integer                                     :: nComputePoints
        real(DP), dimension(nComputePoints)          :: overlandToSewerFlow
