topHSPF_DA <- function(dsn, dsndat, region, event, scn, dsnDAR,
 ...
 source(file.path(dsnDAR, 'trf',   'linearScale.R'))
 ...
 if (!all(rownames(MC[MC$flag,]) %in% names(RF$ana)))
   stop('topHSPF_DA:: uncertain parameters not included in analysis definition')
